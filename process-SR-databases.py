...
def import_worksheets(excel_file_location, openpyxl_db):
    """
    ...
    for excel_worksheet in excel_workbook.worksheets:
        openpyxl_db.append(excel_worksheet)
...
def create_database(openpyxl_db,
    ...
    """
    Processes SRs via `add_data` and adds them to the list database.
    """
    ...
    station_database = import_csv(station_database_file_location, 0)
...
def main():
    ...
    pre_df_db = []
    country_uic_database_file_location = folder_location + '/01_source/data/UIC_lists/country_UIC_list.csv'
    company_uic_database_file_location = folder_location + '/01_source/data/UIC_lists/company_UIC_list.csv'
    station_database_file_location = folder_location + '/01_source/data/station_lists/stations_HU.csv'
    print('Variables initialized!')

    for i, file_location in enumerate(sr_file_locations):
        openpyxl_db = []
        openpyxl_db = import_worksheets(file_location, openpyxl_db)
        if i == 0:
            import_all = True
        else:
            import_all = False
        pre_df_db = create_database(openpyxl_db,
                                    pre_df_db,
                                    import_all,
                                    country_uic_database_file_location,
                                    company_uic_database_file_location,
    ...
    save_path = folder_location + new_file_name
