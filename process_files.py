...
def import_csv(file_location, index_col_loc, values_col_loc):
    ...
    print(f'{file_location} was found and imported!')
    return dictionary
...
def import_worksheets(excel_file_location):
    ...
    """
    print(f'Loading {excel_file_location} started!')
    excel_workbook = load_workbook(excel_file_location, data_only=True)
    print(f'{excel_file_location} loaded!')

    current_sr_file_openpyxl = list(excel_workbook.worksheets)
    print(f'All worksheets imported from {excel_file_location} to current_sr_file_openpyxl!')
...
def add_data(row,
    ...
    def get_station_uic(country_uic, station_name):
        ...
        try:
            ...
            print(f'Error: {station_name} could not be converted to UIC code!')
    ...
    if current_sr.on_station:
        ...
        current_sr.station_to_uic = get_station_uic(current_sr.country_uic, current_sr.station_to_name)
        current_sr.applied_to = 'track'
        if row[3] is None:
    ...
    current_sr.reduced_speed_for_locomotives, current_sr.reduced_speed_for_mus = get_reduced_speeds(row[8])
