...
def predict_category():
    ...
    while True:
        ...
        predicted_category = classifier.predict(vectorizer.transform([text]))[0] if texts else None

        # Ask for user input if the confidence is low or it's the first text input
        confidence = classifier.predict_proba(vectorizer.transform([text])).max() if texts else 0
