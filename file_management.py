from ast import literal_eval    # built-in
import process_files    # built-in
import csv  # built-in
...
def read_objects_csv(file_location):
    ...
    print(f'{file_location} with {len(sr_database)} items was found and imported!')
    return sr_database
...
def mark_eliminated_srs(where, reference):
    ...
    for sr in eliminated_srs:
        if sr.time_to is None:  # do not overwrite already eliminated SRs
            ...
            i += 1
    print(f'{i} SRs marked as eliminated!')
...
def add_new_srs(from_where, to):
    ...
    for sr in from_where:
        to.add(sr)
    print(f'{len(to) - previous_length} new SRs added to sr_database from sr_database_today!')
...
def write_object_list_to_csv(file_location, list_of_objects):
    ...
    print(f'{file_location} with {len(list_of_objects)} items saved!')
